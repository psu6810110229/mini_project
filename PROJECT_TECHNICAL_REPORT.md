...
2. **Backend Layer (ส่วนประมวลผลหลัก)**
    ...
    - **Why NestJS?:**
        ...
        - **TypeScript Support (Type Safety):**
            - *Rationale:* เพื่อป้องกัน Bug ที่เกิดจากข้อมูลผิดประเภท (เช่น ส่ง string ไปคำนวณเลข) ตั้งแต่ตอนเขียนโค้ด ทำให้ระบบมีความเสถียร (Robust) มากขึ้น
            - *Reference:* `create-rental.dto.ts` - มีการประกาศ Class `CreateRentalDto` และใช้ Decorator เช่น `@IsDateString()` หรือ `@IsUUID()` จาก library `class-validator` เพื่อ "บังคับ" ให้ข้อมูลที่ Frontend ส่งเข้ามาต้องถูกต้องตามระเบียบเท่านั้นถึงจะยอมให้ทำงานต่อ
...
3. **Database Layer (ส่วนจัดเก็บข้อมูล)**
    ...
    - **Why PostgreSQL?:**
        - **Relational Integrity:**
            - *Rationale:* ระบบการยืม-คืน มีข้อมูลที่เกี่ยวข้องกันซับซ้อน (ใครยืม? ยืมอะไร? ยืมเมื่อไหร่?) SQL Database จึงเหมาะสมที่สุดในการจัดการความสัมพันธ์เหล่านี้เพื่อไม่ให้ข้อมูลมั่ว
            - *Reference:* `rental.entity.ts` - บรรทัด `@ManyToOne(() => User, (user) => user.rentals)` เป็นการบอก Database ว่า "Rental 1 รายการ ต้องผูกกับ User 1 คนเสมอ" ถ้า User คนนั้นหายไป ข้อมูลการเช่าก็จะรู้ตัว (Foreign Key Constraint)
...
### 1. 🛒 ระบบตระกร้าสินค้า (Shopping Cart & Reservation)
ระบบตะกร้าสินค้าถูกออกแบบให้มีอายุการใช้งาน (Expiry) เพื่อป้องกันการกั๊กของ
- **Logic การทำงาน:**
  1. เมื่อ User กดหยิบของใส่ตะกร้า `addToCart()`, ระบบจะบันทึก `timestamp` ปัจจุบัน และคำนวณ `expiresAt` (เช่น 15 นาทีหลังจากนี้)
  2. ข้อมูลตะกร้าถูกเก็บลง `localStorage` เพื่อให้ไม่หายเวลารีเฟรชหน้าจอ
  3. `useEffect` จะทำงานทุกๆ 1 วินาที เพื่อวนลูปเช็คสินค้าในตะกร้า
...
- ตัวเลขราคาหรือสิทธิ์การจอง **ไม่ได้เชื่อข้อมูลจากตะกร้า 100%**
- เมื่อกด **Confirm**, Backend จะรับแค่ `equipmentId` กับ `date` แล้วไปคำนวณราคาและตรวจสอบสิทธิ์ใหม่ทั้งหมดที่ Server (Server-Side Validation)
