...
const formatDisplayDate = (d: string) => new Date(d).toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' });
...
export default function RentalListButton() {
    ...
    const checkForOverlaps = async (): Promise<boolean> => {
        ...
        try {
            ...
            return false;
        } catch (err: any) { setError(err.response?.data?.message || 'Failed to check availability'); return false; }
    ...
    const handleConfirmClick = async () => {
        ...
        if (s < now) return setError('Start date cannot be in the past');
        if (e <= s) return setError('End date must be after start date');
        if (!rentalReason.trim()) return setError('Please provide a reason for your rental request');
    ...
    const submitRentals = async (allowOverlap = false) => {
        ...
        } catch (err: any) { setError(err.response?.data?.message || 'Failed to submit rental requests'); }
                    ...
                    <div className="absolute right-0 top-0 h-full w-full max-w-lg backdrop-blur-2xl bg-gradient-to-b from-slate-900/90 to-slate-800/90 shadow-2xl animate-slide-in-right border-l border-white/10">
                        <div className="flex flex-col h-full">
                            <div className="flex items-center justify-between p-6 border-b border-white/10 bg-gradient-to-r from-blue-600/20 to-purple-600/20">
                                <div><h2 className="text-2xl font-bold text-white flex items-center gap-3"><div className="p-2 bg-white/10 rounded-xl"><ShoppingBag className="h-6 w-6" /></div>Rental List</h2><p className="text-white/60 text-sm mt-1">{cartItems.length} item{cartItems.length !== 1 ? 's' : ''}</p></div>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                                {cartItems.length === 0 ? (
                                    <div className="text-center py-16"><div className="w-24 h-24 mx-auto mb-6 rounded-full bg-white/5 flex items-center justify-center"><ShoppingBag className="h-12 w-12 text-white/30" /></div><h3 className="text-xl font-semibold text-white mb-2">Rental List is Empty</h3><p className="text-white/50">Select equipment from the list to start renting</p></div>
                                ) : (<>
                                    <div className="space-y-3">
                                        <h3 className="text-sm font-semibold text-white/70 uppercase tracking-wider flex items-center gap-2"><ClipboardList className="w-4 h-4" />Selected Equipment</h3>
                                        {cartItems.map((item, i) => (
                                            <div key={item.itemId} className="group backdrop-blur-xl bg-white/5 rounded-2xl p-4 border border-white/10 hover:border-white/20 animate-fade-in" style={{ animationDelay: `${i * 100}ms` }}>
                                                <div className="flex gap-4 items-center">
                                                    ...
                                                    <div className="flex-1 min-w-0"><h4 className="font-semibold text-white truncate">{item.equipmentName}</h4><p className="text-sm text-white/50">ID: {item.itemCode}</p><div className="flex items-center gap-2 mt-2"><Clock className="h-3 w-3 text-amber-400" /><span className="text-xs font-mono text-amber-400">{formatTimeRemaining(item.expiresAt)}</span></div></div>
                                    ...
                                    <div className="backdrop-blur-xl bg-blue-500/10 rounded-2xl p-5 border border-blue-500/20 space-y-4">
                                        <h3 className="font-semibold text-white flex items-center gap-2"><Calendar className="h-5 w-5 text-blue-400" />Rental Period</h3>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div><label className="block text-xs text-white/50 mb-2 uppercase">Start</label><input type="datetime-local" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-xl p-3 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50" style={{ colorScheme: 'dark' }} /></div>
                                            <div><label className="block text-xs text-white/50 mb-2 uppercase">End</label><input type="datetime-local" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-xl p-3 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50" style={{ colorScheme: 'dark' }} /></div>
                                        </div>
                                    </div>
                                    <div className="backdrop-blur-xl bg-amber-500/10 rounded-2xl p-5 border border-amber-500/20 space-y-3">
                                        <h3 className="font-semibold text-white flex items-center gap-2"><FileText className="h-5 w-5 text-amber-400" />Reason for Rental <span className="text-red-400">*</span></h3>
                                        <textarea value={rentalReason} onChange={e => setRentalReason(e.target.value)} placeholder="e.g., Event recording, Final project..." rows={3} className="w-full bg-white/10 border border-white/20 rounded-xl p-4 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-amber-500/50 resize-none" />
                                    </div>
                            ...
                            {cartItems.length > 0 && (
                                <div className="p-6 border-t border-white/10 bg-gradient-to-t from-slate-900/50 space-y-3">
                                    <button onClick={handleConfirmClick} disabled={isConfirmDisabled} className="w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white py-4 rounded-2xl font-bold text-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3">
                                        {checkingOverlap || submitting ? <><svg className="animate-spin h-5 w-5" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Processing...</> : <><CheckCircle className="h-5 w-5" />Confirm Rental</>}
                                    </button>
                                    <button onClick={() => showConfirmation('clearAll')} disabled={submitting} className="w-full bg-white/5 hover:bg-white/10 text-white/70 hover:text-white py-3 rounded-xl font-medium border border-white/10">Clear All Items</button>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {showOverlapModal && overlapWarning && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center">
                    ...
                    <div className="relative backdrop-blur-2xl bg-gradient-to-b from-slate-800/95 to-slate-900/95 rounded-3xl shadow-2xl max-w-md w-full mx-4 p-6 animate-scale-in border border-white/20">
                        <div className="flex items-center gap-3 mb-4"><div className="p-3 bg-amber-500/20 rounded-2xl"><AlertTriangle className="h-7 w-7 text-amber-400" /></div><div><h3 className="text-xl font-bold text-white">Overlap Detected</h3><p className="text-sm text-white/60">Someone else has requested this time slot</p></div></div>
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => { setShowOverlapModal(false); setOverlapWarning(null); }} className="flex-1 px-4 py-3 rounded-xl font-medium text-white bg-white/10 hover:bg-white/20 border border-white/20">Change Dates</button>
                            <button onClick={() => { setShowOverlapModal(false); submitRentals(true); }} disabled={submitting} className="flex-1 px-4 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 shadow-lg disabled:opacity-50">{submitting ? 'Submitting...' : 'Proceed Anyway'}</button>
                        </div>
                    </div>
                </div>
            )}

            <ConfirmModal isOpen={showConfirmModal} title="Confirm Action" message={confirmAction?.type === 'removeItem' ? 'Remove this item from the rental list?' : 'Clear all items from the rental list?'} variant="warning" confirmLabel="Confirm" onConfirm={handleConfirmedAction} onCancel={() => { setShowConfirmModal(false); setConfirmAction(null); }} />
        </>
    );
}
