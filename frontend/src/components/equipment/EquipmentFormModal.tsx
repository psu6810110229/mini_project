        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50">
            <div className="backdrop-blur-2xl bg-gradient-to-b from-slate-800/95 to-slate-900/95 rounded-3xl w-full max-w-md border border-white/20 shadow-2xl p-6 animate-scale-in">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-bold text-white">{editingItem ? 'Edit Equipment' : 'Add New Equipment'}</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            {editingItem && <p className="text-xs text-white/50 mt-1">Cannot change after creation</p>}
                        </div>
                        <div>
                    ...
                    {/* Image Upload */}
                    <div>
                        <label className="block text-sm text-white mb-2 font-semibold">Equipment Image</label>
                    ...
                    <div className="flex gap-3 pt-6">
                        <button type="button" onClick={onClose} disabled={saving} className="flex-1 px-4 py-3 backdrop-blur-xl bg-white/10 hover:bg-white/20 disabled:opacity-50 rounded-xl text-white font-medium border border-white/20">Cancel</button>
                        <button type="submit" disabled={saving} className="flex-1 px-4 py-3 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-xl font-semibold shadow-lg disabled:opacity-50">{saving ? 'Saving...' : 'Save'}</button>
                    </div>
                </form>
            </div>
        </div>
