...
export default function UploadEvidenceModal({ isOpen, rentalId, imageType, onClose, onSuccess }: UploadEvidenceModalProps) {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (selected) {
            if (selected.size > 5 * 1024 * 1024) return setError('File too large (max 5MB)');
            setFile(selected);
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative backdrop-blur-2xl bg-slate-900/90 rounded-3xl border border-white/20 p-6 max-w-md w-full shadow-2xl animate-scale-in max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-white capitalize">{imageType} Evidence</h3>
                ...
                <div className="space-y-4">
                    ...
                    >
                        ...
                        {preview ? (
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-white/40 text-xs">Max 5MB (JPG, PNG)</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div>
                        <label className="text-sm font-medium text-white/70 mb-2 block flex items-center gap-2">
                            <FileText className="w-4 h-4" /> Note <span className="text-white/30 text-xs font-normal">(Optional)</span>
                        </label>
                        <textarea
                            value={note}
                            onChange={(e) => setNote(e.target.value)}
                            placeholder="Add details about condition or location..."
