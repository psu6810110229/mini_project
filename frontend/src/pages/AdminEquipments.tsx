...
export default function AdminEquipments() {
    ...
    const fetchEquipments = async () => {
        ...
        } catch { setErrorMessage('Failed to load equipments.'); }
    ...
    const handleDelete = async (id: string) => {
        try { setSaving(true); await apiClient.delete(`/equipments/${id}`); showSuccess('Equipment deleted!'); fetchEquipments(); }
    ...
    const handleSubmit = async (data: { name: string; category: string; stockQty: number; status: string }, imageFile: File | null) => {
        try {
            setSaving(true);
            let equipment;
            if (editingItem) { equipment = await apiClient.patch(`/equipments/${editingItem.id}`, data); showSuccess('Equipment updated!'); }
            else { equipment = await apiClient.post('/equipments', data); showSuccess('Equipment added!'); }
    ...
    const handleImageUpload = async (equipmentId: string, file: File) => {
        ...
        try {
            ...
            showSuccess('Image uploaded!'); fetchEquipments();
        } catch (err: any) { showError(err.response?.data?.message || 'Failed to upload image'); }
        finally { setUploadingImage(null); }
    };

    const handleStatusChange = async (itemId: string, newStatus: EquipmentItemStatus) => {
        try { setSaving(true); await apiClient.patch(`/equipments/items/${itemId}/status`, { status: newStatus }); showSuccess('Status updated!'); fetchEquipments(); }
        catch (err: any) { showError(err.response?.data?.message || 'Failed to update status'); }
        finally { setSaving(false); }
    };
    ...
    return (
        <div className="p-4 md:p-8 max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-3"><Settings className="w-8 h-8 text-white" /><h1 className="text-3xl md:text-4xl font-bold text-white" style={{ textShadow: '2px 2px 8px rgba(0,0,0,0.8)' }}>Manage Equipment</h1></div>
                <button onClick={() => { setEditingItem(null); setIsModalOpen(true); }} disabled={saving} className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white px-6 py-3 rounded-2xl font-semibold shadow-lg transition-all disabled:opacity-50"><Plus className="h-5 w-5" /> Add New Equipment</button>
            </div>
            ...
            {equipments.length === 0 ? <EmptyState icon={Package} message="No equipment found. Start by adding new equipment!" /> : (
                <div className="space-y-4">
                    ...
                    {filteredEquipments.length === 0 && <EmptyState icon={Package} message="No equipment matches the filter criteria." />}
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center">
                    ...
                    <div className="relative backdrop-blur-2xl bg-gradient-to-b from-slate-800/95 to-slate-900/95 rounded-3xl shadow-2xl max-w-sm w-full mx-4 p-6 animate-scale-in border border-white/20">
                        <div className="text-center mb-6">
                            ...
                            <p className="text-white/60">{confirmAction.type === 'delete' ? `Delete "${confirmAction.name}"?` : `Change ${confirmAction.name}?`}</p>
                            {confirmAction.type === 'delete' && <p className="text-red-400 text-sm mt-2">This cannot be undone.</p>}
                        </div>
                        <div className="flex gap-3">
                            <button onClick={() => setShowConfirmModal(false)} className="flex-1 px-4 py-3 rounded-xl font-medium text-white bg-white/10 hover:bg-white/20 border border-white/20">Cancel</button>
                            ...
                            }} className={`flex-1 px-4 py-3 rounded-xl font-bold text-white shadow-lg ${confirmAction.type === 'delete' ? 'bg-gradient-to-r from-red-500 to-red-600' : 'bg-gradient-to-r from-blue-500 to-blue-600'}`}>Confirm</button>
                        </div>
                    </div>
                </div>
            )}
